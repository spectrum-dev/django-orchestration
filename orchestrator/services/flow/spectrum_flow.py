            ...
            for task_to_be_run in task:
                ...
                if (len(self.dependency_graph.adjacency_list[task_to_be_run]) == 0):
                    ...
                    for item in set(blocks_found):
                        ...
                        if item_block_data["data"]["metadata"]["blockType"] not in assembled_dependency_list_from_flow:
                            assembled_dependency_list_from_flow[item_block_data["data"]["metadata"]["blockType"]] = 0
                        assembled_dependency_list_from_flow[item_block_data["data"]["metadata"]["blockType"]] += 1

                    for required in block_registry_data.validations["input"]["required"]:
                        is_valid = is_valid and (assembled_dependency_list_from_flow[required["blockType"]] == required["number"])
